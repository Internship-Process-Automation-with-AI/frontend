...
import { requestInterceptor } from '../archieves/api.js'
  ...
  useEffect(() => {
    ...
    const unsubscribe = requestInterceptor.subscribe(activeRequests => {
      ...
      const overallProgress = hasActiveRequests
        ? activeRequests.reduce((sum, req) => sum + req.progress, 0) /
...
export const useOperationLoading = operationType => {
  ...
  const operationRequests = activeRequests.filter(
    req => req.type === operationType
  )
  const isActive = operationRequests.length > 0
  const progress = isActive
    ? operationRequests.reduce((sum, req) => sum + req.progress, 0) /
...
export const useDocumentProcessing = () => {
  const { isActive, progress, currentRequest } =
    useOperationLoading('process_document')
